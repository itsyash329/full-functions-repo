...
class Bot(Client):
    ...
    async def start(self):
        ...
        try:
            ...
            self.shortners = shortner_settings.get('shortners') or [{"id": 1, "url": shortner_settings.get('short_url', SHORT_URL), "api": shortner_settings.get('short_api', SHORT_API), "enabled": True}]
            self.shortner_index = shortner_settings.get('shortner_index', 0)
            # Legacy compatibility for existing code
            self.short_url = self.shortners[0]['url']
            self.short_api = self.shortners[0]['api']
            ...
            self.shortners = [{"id": 1, "url": SHORT_URL, "api": SHORT_API, "enabled": True}]
            self.shortner_index = 0
        ...
        try:
            ...
            test = await self.send_message(chat_id = db_channel.id, text = "Testing Message by @BRUCE_WAYNE_01B")
            await test.delete()
