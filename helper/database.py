...
class MongoDB:
    ...
    async def create_short_start_token(self, payload: str) -> str:
        """Create a compact Telegram start token for a payload."""
        import secrets
        for _ in range(20):
            token = secrets.token_urlsafe(6).replace("-", "").replace("_", "")[:8]
            exists = await self.user_data.find_one({"_id": f"short_start:{token}"})
            if not exists:
                await self.user_data.insert_one({"_id": f"short_start:{token}", "payload": payload})
                return token
        raise RuntimeError("Could not generate a unique short start token")

    async def get_short_start_payload(self, token: str):
        data = await self.user_data.find_one({"_id": f"short_start:{token}"})
...
# ---- Access / verification extension methods (attached to MongoDB for backward compatibility) ----
async def _get_access_settings(self):
    d = await self.db['access_settings'].find_one({'_id':'main'}) or {}
    defaults={'credit_price':1,'credit_reward':1,'token_hours':24,'shortener_expiry':10,'min_verify_seconds':30,'purchase_url':''}
    defaults.update({k:v for k,v in d.items() if k!='_id'})
    return defaults
async def _update_access_setting(self,key,value):
    await self.db['access_settings'].update_one({'_id':'main'},{'$set':{key:value}},upsert=True)
async def _get_access_mode(self):
    d=await self.db['access_settings'].find_one({'_id':'mode'}) or {}; return d.get('mode','shortener')
async def _set_access_mode(self,mode):
    if mode not in ('shortener','credit','token','free'): raise ValueError('invalid mode')
    await self.db['access_settings'].update_one({'_id':'mode'},{'$set':{'mode':mode}},upsert=True)
...
async def _list_credit_users(self):
    cur=self.user_data.find({'_id':{'$type':'long'},'credit':{'$gt':0}}).sort('credit',-1)
    return [x async for x in cur]
async def _create_pending_verification(self,uid,payload,kind):
    import secrets
...
async def _get_pending_verification(self,token): return await self.db['pending_verifications'].find_one({'_id':token})
async def _consume_pending_verification(self,token): await self.db['pending_verifications'].update_one({'_id':token},{'$set':{'verified':True,'verified_at':datetime.now()}})
async def _set_token_access(self,uid,hours): await self.db['token_access'].update_one({'_id':uid},{'$set':{'until':datetime.now()+timedelta(hours=hours)}},upsert=True)
async def _get_token_access(self,uid):
    d=await self.db['token_access'].find_one({'_id':uid}); return d.get('until') if d else None
MongoDB.get_access_settings=_get_access_settings; MongoDB.update_access_setting=_update_access_setting
MongoDB.get_access_mode=_get_access_mode; MongoDB.set_access_mode=_set_access_mode
