...
def force_sub(func):
    ...
    async def wrapper(client: Client, message: Message):
        ...
        # Organize channel buttons in rows of 2
        button_rows = [buttons[i:i + 2] for i in range(0, len(buttons), 2)]

        # Add "Try Again" button on its own wide line at the bottom
        from_link = message.text.split(" ")
        if len(from_link) > 1:
            try_again_link = f"https://t.me/{client.username}/?start={from_link[1]}"
            button_rows.append([InlineKeyboardButton("🔄 Try Again", url=try_again_link)])

        # Construct markup cleanly
        buttons_markup = InlineKeyboardMarkup(button_rows) if button_rows else None
...
DEL_MSG = """<b><blockquote>⚠️Yᴏᴜʀ ғɪʟᴇs ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ᴡɪᴛʜɪɴ <a href="https://t.me/{username}">{time}</a>. Sᴏ ᴘʟᴇᴀsᴇ ғᴏʀᴡᴀʀᴅ ᴛʜᴇᴍ ᴛᴏ ᴀɴʏ ᴏᴛʜᴇʀ ᴘʟᴀᴄᴇ ғᴏʀ ғᴜᴛᴜʀᴇ ᴀᴠᴀɪʟᴀʙɪʟɪᴛʏ.</b></blockquote>"""
    ...
    try:
        if transfer:
            try:
                ...
                await temp.edit_text(text=f"<b>›› Pʀᴇᴠɪᴏᴜs Mᴇssᴀɢᴇ ᴡᴀs Dᴇʟᴇᴛᴇᴅ 🗑️\n<b><blockquote>Gᴇᴛ ɪᴛ ᴀɢᴀɪɴ ➜ <a href={link}>{name}</a></b></blockquote></b>", reply_markup=InlineKeyboardMarkup(button), disable_web_page_preview = True)
        ...
        if transfer_link:
            try:
                ...
                await notification_msg.edit_text(
                    text=f"<b>›› Pʀᴇᴠɪᴏᴜs Mᴇssᴀɢᴇ ᴡᴀs Dᴇʟᴇᴛᴇᴅ 🗑️\n<b><blockquote>Gᴇᴛ ɪᴛ ᴀɢᴀɪɴ ➜ <a href={link}>{name}</a></b></blockquote></b>",
